...
class ResponseDecoder(nn.Module):
    def __init__(self, embedding, vocab_size_oov, Wgen = None, dropout=0.):
        ...
        self.vsize_oov = vocab_size_oov
        self.hidden_size = cfg.hidden_size
        self.degree_size = cfg.degree_size

        gru_input_size = self.hidden_size + self.embed_size + cfg.pointer_dim
        if cfg.enable_bspn:
        ...
        self.Wgen = nn.Linear(self.hidden_size, cfg.vocab_size) if not Wgen else Wgen
        self.attn_usdx = Attn(self.hidden_size)
        if cfg.enable_bspn:
            self.attn_bspn = self.attn_usdx if cfg.attn_param_share else Attn(self.hidden_size)
        if cfg.enable_aspn:
            self.attn_aspn = self.attn_usdx if cfg.attn_param_share else Attn(self.hidden_size)

        self.cp_usdx = Copy(self.hidden_size)
        if cfg.enable_bspn:
            self.cp_bspn = self.cp_usdx if cfg.copy_param_share else Copy(self.hidden_size)
        if cfg.enable_aspn:
            self.cp_aspn = self.cp_usdx if cfg.copy_param_share else Copy(self.hidden_size)
